...
class TestManualEntriesImportEndpoint:
    ...
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_success(self, mock_project_handler):
        """Test successful import of manual entries."""
        mock_project_handler._get.return_value = self.mock_project
        ...
        assert id_mapping["mapped_entries"] >= 0
        assert id_mapping["unmapped_entries"] >= 0
    
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_project_not_found(self, mock_project_handler):
        """Test import with non-existent project."""
        from structure_comparer.data.exceptions import ProjectNotFound
        mock_project_handler._get.side_effect = ProjectNotFound("Project not found")
    ...
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_invalid_yaml(self, mock_project_handler):
    ...
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_no_file(self, mock_project_handler):
    ...
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_migration_error(self, mock_project_handler):
    ...
    @patch('structure_comparer.serve.project_handler')
    def test_import_manual_entries_save_error(self, mock_project_handler):
        """Test import with save error."""
        mock_project = Mock()
        mock_project_handler._get.return_value = mock_project
        # Mock the ManualEntries.write() to raise an exception
        with patch('structure_comparer.manual_entries.ManualEntries.write', side_effect=Exception("Failed to save")) as mock_write:
            legacy_data = {"test": {"field": {"classification": "use"}}}
            yaml_file = self.create_test_yaml_file(legacy_data)
